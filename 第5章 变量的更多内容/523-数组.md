...
```javascript int[][] jaggedIntArray;```

&emsp;&emsp;但初始化这样的数组不像初始化多维数组那样简单，例如不能采用以下的声明方式：

```javascript jaggedIntArray = new int[3][4];```

&emsp;&emsp;即使这样做了，也不是很有效，因为使用简单的多维数组可以较轻松地取得相同的结果。也不能使用下面的代码：

```javascript jaggedIntArray = { {1, 2, 3}, { 1 }, { 1, 2 } }; ❌```

&emsp;&emsp;有两种方式：可以初始化包含其他数组的数组（为清晰起见，称其为子数组），然后依次初始化子数组：

```javascript jaggedIntArray = new int[2][]; jaggedIntArray[0] = new int[3]; jaggedIntArray[1] = new int[4];```

&emsp;&emsp;也可以使用上述字面值赋值的一种改进形式：

```javascript jaggedIntArray = new int[3][] { new int[] { 1, 2, 3 }, new int[] { 1 }, new int[] { 1, 2 } };```

&emsp;&emsp;也可以进行简化，把数组的初始化和声明放在同一行上，如下所示：

```javascript int[][] jaggedIntArray = { new int[] { 1, 2, 3 }, new int[] { 1 }, new int[] { 1, 2 } };

```

&emsp;&emsp;可以对锯齿数组使用 `foreach` 循环，但通常需要使用嵌套的 `foreach` 循环才能得到实际数据。例如，假定下述锯齿数组包含 10 个数组，每个数组又包含一个整数数组，其元素是 1～10 的约数：

```javascript int[][] divisors1To10 = { new int[] { 1 }, new int[] { 1, 2 }, new int[] { 1, 3 }, new int[] { 1, 2, 4 }, new int[] { 1, 5 }, new int[] { 1, 2, 3, 6 }, new int[] { 1, 7 }, new int[] { 1, 2, 4, 8 }, new int[] { 1, 3, 9 }, new int[] { 1, 2, 5, 10 } };```

&emsp;&emsp;下面的代码会失败：

```javascript foreach ( int divisor in divisors1To10 ) { Console.WriteLine(divisor); }```

&emsp;&emsp;这是因为数组 `divisors1To10` 包含 `int[]` 元素，而不是 `int` 元素。正确的做法是循环遍历每个子数组和数组本身：

```javascript foreach ( int[] divisorsOfInt in divisors1To10 ) { foreach ( int divisor in divisorsOfInt ) { Console.WriteLine(divisor); } }```
