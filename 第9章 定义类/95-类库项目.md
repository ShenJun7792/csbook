...
>&emsp;&emsp;这个示例创建了两个项目，一个是类库项目，另一个是控制台应用程序项目。类库项目 `Ch09ClassLib` 包含两个类： `MyExternalClass` （可公开访问）和 `MyInternalClass` （只能在内部访问）。注意 ⚠️，默认情况下，会隐式将类确定为供内部访问，因为它没有访问修饰符。最好明确指定可访问性，因为这会使代码更便于理解，所以指令增加了 `interanl` 关键字
